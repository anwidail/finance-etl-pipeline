...
def reconcile_rows(
    ...
    if abs(diff) > tolerance:
        raise ValueError(
            f"unbalanced cash_in transform for source_id={source_id}: "
            f"total_debit={total_debit}, total_credit={total_credit}, diff={diff}"
        )


def validate_cash_in_payload(payload: Dict[str, Any]) -> None:
    if not isinstance(payload, dict):
        raise TypeError("payload must be a dict")

    end_point = payload.get("end_point")
    if end_point != "cash_ins":
...
def transform_cash_in_from_body(raw_body: Any) -> List[Dict[str, Any]]:
    payload = parse_json_body(raw_body)
    return transform_cash_in(payload)


def transform_cash_in(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Journal pattern (money received):
    1. Debit cash / bank from data.cash
    2. Credit each source-of-funds account from data.line_items[]
    """
    validate_cash_in_payload(payload)
    ...
    # 2) Credit each line item account (source of funds)
    for line in data.get("line_items", []) or []:
        ...
        rows.append({
            **base,
            ...
            "debit": Decimal("0"),
            "credit": line_amount,
            "realization": compute_realization(acc_code, Decimal("0"), line_amount),
    ...
    if not rows:
        raise ValueError(f"no rows generated for cash_in source_id={data.get('id')}")

    reconcile_rows(rows, source_id=str(data["id"]))
    return rows


def transform_cash_in_source_record(source_record: Dict[str, Any]) -> List[Dict[str, Any]]:
    rows = transform_cash_in_from_body(source_record["body"])
