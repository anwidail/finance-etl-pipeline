...
def transform_receivable_payment(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for line in data.get("line_items", []) or []:
        ...
        # 3b) Credit A/R
        # priority: payment.amount -> payable.amount -> amount
        ar_cleared_amount = to_decimal(
            get_nested(line, "payment", "amount")
            if get_nested(line, "payment", "amount") is not None
            else get_nested(line, "payable", "amount")
            if get_nested(line, "payable", "amount") is not None
            else line.get("amount")
        )

        ar_code = to_str(ar_acc.get("code"))

        if ar_cleared_amount != 0:
            rows.append({
                **line_base,
                "note": "Accounts Receivable (Payment)",
                "debit": Decimal("0"),
                "credit": ar_cleared_amount,
                "realization": compute_realization(ar_code, Decimal("0"), ar_cleared_amount),
